...
class OpeningResultServiceTests(unittest.TestCase):
    ...
    def test_global_collection_keeps_shared_source_and_matches_user_afterward(self):
    ...
    def test_new_profile_match_backfills_missing_entry_details_on_next_collection(self):
        ...
        collect_opening_results(self.db, self.request, first_client)

        round_row = self.db.scalar(select(BidOpeningRoundModel))
        self.assertEqual(first_client.fetch_entry_call_count, 0)
        ...
        with self.assertRaises(HTTPException) as raised:
            export_results_sheet(
                ExportOpeningResultsSheetRequest(result_ids=[round_row.id]),
                auth=self.auth,
                db=self.db,
            )
        self.assertEqual(raised.exception.status_code, 409)
        self.assertEqual(
            raised.exception.detail["pending_detail_result_ids"],
            [round_row.id],
        )

        source = self.completed_summary()
        backfill_client = StubOpeningResultClient(
            [],
            entries={
                (source["bidNtceNo"], source["bidNtceOrd"], source["rbidNo"]): self.entries()
            },
        )
        next_request = CollectOpeningResultsRequest(
            start_at=datetime(2026, 7, 15, tzinfo=timezone.utc),
            end_at=datetime(2026, 7, 16, tzinfo=timezone.utc),
        )
        collect_opening_results(self.db, next_request, backfill_client)

        self.db.refresh(round_row)
        self.assertEqual(backfill_client.fetch_entry_call_count, 1)
    ...
    def test_list_route_allows_blank_base_amount_for_nonpriced_notice(self):
        collect_opening_results(self.db, self.request, self.make_client())
        self.add_bid_notice(
            base_amount=None,
        ...
        self.assertIsNone(response.items[0].base_amount)
    ...
    def test_scheduled_collection_runs_once_per_twelve_hour_slot(self):
        client = self.make_client()
        ...
        self.assertEqual(first.run_key, "SERVICE:2026071512")
        self.assertEqual(first.window_end - first.window_start, timedelta(hours=12))
    ...
    def test_schedule_window_uses_kst_midnight_and_noon_boundaries(self):
        kst = timezone(timedelta(hours=9))
        cases = [
            (
                datetime(2026, 7, 15, 11, 59, tzinfo=kst),
                "SERVICE:2026071500",
                datetime(2026, 7, 15, 0, 0, tzinfo=kst),
            ),
            (
            ...
            (
                datetime(2026, 7, 16, 0, 0, tzinfo=kst),
                "SERVICE:2026071600",
                datetime(2026, 7, 16, 0, 0, tzinfo=kst),
            ),
        ]

        for current, expected_key, expected_local_end in cases:
            with self.subTest(current=current):
                ...
                self.assertEqual(window_end - window_start, timedelta(hours=12))
    ...
    def test_next_collection_slot_does_not_duplicate_previous_results(self):
        client = self.make_client()
        first_now = datetime(2026, 7, 15, 3, 17, tzinfo=timezone.utc)
    ...
    def test_scheduled_collection_catches_up_from_last_successful_window(self):
        now = datetime(2026, 7, 16, 3, 17, tzinfo=timezone.utc)
        _, _, current_window_end = build_scheduled_collection_window(now)
        previous_window_end = current_window_end - timedelta(hours=36)
        self.db.add(
            BidOpeningCollectionRunModel(
                run_key="SERVICE:previous-success",
                business_type="SERVICE",
                window_start=previous_window_end - timedelta(hours=12),
                window_end=previous_window_end,
                status="SUCCESS",
                started_at=previous_window_end - timedelta(hours=12),
    ...
    def test_scheduled_catchup_window_is_capped_at_fourteen_days(self):
        now = datetime(2026, 7, 20, 3, 17, tzinfo=timezone.utc)
        _, _, current_window_end = build_scheduled_collection_window(now)
        ...
        self.db.add(
            BidOpeningCollectionRunModel(
                ...
                business_type="SERVICE",
                window_start=old_window_end - timedelta(hours=12),
                window_end=old_window_end,
                status="SUCCESS",
                started_at=old_window_end - timedelta(hours=12),
    ...
    def test_scheduler_route_returns_conflict_while_same_slot_is_running(self):
        running_response = ScheduledCollectOpeningResultsResponse(
            run_key="SERVICE:2026071512",
            window_start=datetime(2026, 7, 15, 0, 0, tzinfo=timezone.utc),
...
class OpeningResultRouterTests(unittest.TestCase):
    ...
    def test_sheet_writer_updates_and_inserts_selected_rows_in_one_batch(self):
        ...
        self.assertEqual(
            service.calls[2][1]["body"]["data"][0]["range"],
            "'개찰결과'!A2:Q2",
        )
        self.assertEqual(
            service.calls[2][1]["body"]["data"][1]["range"],
            "'개찰결과'!A3:Q3",
        )
