...
class ScraperNoticePersistenceTests(unittest.TestCase):
    ...
    def test_missing_official_base_amount_is_exported_as_blank(self):
        notice = ScraperNotice(
            notice_id="R26BK00000001",
            title="비예가 AI 구축 용역",
            estimated_price="44501100",
            bid_notice_no="R26BK00000001",
            bid_notice_ord="00",
    ...
    def test_worker_enriches_base_amount_and_region_from_official_operations(self):
        notice = NoticeRow(
            notice_id="R26BK00000001",
            title="AI 교육 운영 용역",
            bid_notice_no="R26BK00000001",
        ...
        with patch.dict(
            ...
            side_effect=[base_response, region_response],
        ) as get_mock:
        ...
        self.assertTrue(
            get_mock.call_args_list[0].args[0].endswith(
                "/getBidPblancListInfoServcBsisAmount"
            )
        )
        self.assertTrue(
            get_mock.call_args_list[1].args[0].endswith(
                "/getBidPblancListInfoPrtcptPsblRgn"
            )
        )
        self.assertEqual(
            get_mock.call_args_list[0].kwargs["params"]["inqryDiv"],
            "2",
        )
        self.assertEqual(
            get_mock.call_args_list[1].kwargs["params"]["inqryDiv"],
            "2",
        )

    def test_worker_skips_base_amount_lookup_for_nonpriced_notice(self):
        notice = NoticeRow(
            notice_id="R26BK00000001",
        ...
        self.assertIsNone(rows[0].base_amount)
