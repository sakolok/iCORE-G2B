...
def build_sheet_row(
    ...
    ranked: dict[int, BidOpeningEntryModel] = {}
    for entry in sorted(entries, key=lambda item: (item.rank is None, item.rank or 0, item.id)):
        if entry.rank is not None and 1 <= entry.rank <= 5 and entry.rank not in ranked:
            ranked[entry.rank] = entry
    row: list[str | int | float] = [
        ...
        _sheet_number(context.base_amount if context else None),
...
def _has_complete_notice_context(context: BidNoticeSheetContext | None) -> bool:
    if context is None:
        return False
    has_valid_base_amount = bool(
        context.base_amount is not None
        or (context.prearranged_price_decision_method or "").strip() == "비예가"
    )
    return bool(
