...
def collect_opening_results(
    ...
    *,
    ...
    try:
        ...
        db.commit()
        pending_detail_keys = set(
            db.scalars(
                select(BidOpeningRoundModel.external_key)
                .join(
                    UserOpeningResultMatchModel,
                ...
                .where(
                    ...
                    or_(
                        BidOpeningRoundModel.opened_at
                        >= datetime.now(timezone.utc) - timedelta(days=FRONT_LIST_DAYS),
                        BidOpeningRoundModel.collected_at
                        >= datetime.now(timezone.utc) - timedelta(days=FRONT_LIST_DAYS),
                    ),
                )
            )
            ...
            matched_keys = set(
                db.scalars(
                    select(UserOpeningResultMatchModel.result_external_key).where(
                        UserOpeningResultMatchModel.is_current_match.is_(True),
                        UserOpeningResultMatchModel.result_external_key.in_(
                            list(entry_candidates)
                        ),
                    )
                )
            )
            for external_key, source in entry_candidates.items():
                if external_key not in matched_keys:
                    continue
                round_row = db.scalar(
                    select(BidOpeningRoundModel).where(
                        BidOpeningRoundModel.external_key == external_key
...
def build_scheduled_collection_window(
    ...
    slot_hour = 12 if local_current.hour >= 12 else 0
    ...
    window_start = window_end - timedelta(hours=COLLECTION_INTERVAL_HOURS)
