...
DB/API 호환을 위해 필드명은 `base_amount`를 유지하지만, 개찰결과 화면과 Sheet에서는
공식 `presmptPrce + VAT`로 계산한 사업금액을 저장한다. `estimated_price`는 추정가격으로
별도 유지하며 사업금액으로 그대로 복사하지 않는다.
...
| `region_restriction_api_status` | 값 확인 `API_VALUE`, 정상 빈 응답 `API_EMPTY`, 호출·응답 오류 `API_ERROR`, 다른 공고·차수 응답 `ORDER_MISMATCH` |
| `is_two_stage_bid` | 검증된 공식 입찰·계약·낙찰방법명에 `2단계` 포함 여부 |

사업금액은 공고 본문의 추정가격과 부가세로 계산하고, 참가가능지역 URL은
`G2B_SOURCE_URL`의 서비스 경로에서 자동으로 파생한다. 별도 게이트웨이를 쓰는 환경은
`G2B_REGION_SOURCE_URL`로 지역 오퍼레이션 URL을 덮어쓸 수 있다. 지역 제한 공식값 미확인 상태는
추정값으로 채우지 않고 `NULL`로 유지하되 Sheet 반영은 허용하고 지역제한 셀만 비운다. 그 밖의 공식
필수값은 미확인 상태에서 Sheet 실제 반영을 차단한다. `전자입찰`처럼 2단계 여부를
...
기존 `region_restriction="없음"`이면서 API 상태가 없는 행도 미확인으로 취급해 Sheet에는 빈 셀로 출력한다.
기존 Sheet의 `기초금액` 헤더는 다음 실제 반영 시 `사업금액`으로 자동 교체한다.
...
호출하지 않는다. 공식 공고가 없거나 지역제한을 제외한 필수 필드가 비어 있거나 같은 공고번호·차수의
행이 여러 건이면 Google API 호출 전에 전체 쓰기를 409로 차단한다.

## 공통 원본과 사용자별 매칭

정기 수집은 사용자 수와 관계없이 한 번만 실행하며 `g2b_opening_rounds`와
...
DB의 과거 결과를 삭제하지 않는다. 목록과 상세는 입찰공고 DB의 공식 사업명·수요기관·사업금액·
...
선택 결과가 없거나 입찰공고 컨텍스트의 사업금액·제안마감·지역제한여부·2단계 입찰 여부가
...
`08:00`, `11:00`, `14:00`, `17:00` KST에 호출하며 Sheet에는 기록하지 않고 DB만 갱신한다.
...
일 4회 스케줄을 사용한다. 개찰 목록은 입력일시, 최종 낙찰 목록은 등록일시를 기준으로
...
`hd`와 이메일 주소가 허용 도메인인지 확인한다. `iceu.kr` 또는 `iceu.co.kr`의 검증된 계정은
최초 로그인에서 일반 사용자, 기본 조직 멤버십, 비활성·빈 개인 키워드 프로필을 한 번 생성한다.
