...
class PreSpecificationModel(Base):
    ...
    bid_notice_no: Mapped[str | None] = mapped_column(
    ...
    demand_agency_name: Mapped[str | None] = mapped_column(
        String(240), nullable=True, index=True
    )
    ordering_agency_name: Mapped[str | None] = mapped_column(
    ...
    opinion_deadline: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, index=True
    )
...
class PreSpecificationSnapshotModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "bf_spec_rgst_no",
            "payload_hash",
            name="uq_pre_spec_snapshot_version",
        ),
    )
...
class UserPreSpecificationProfileModel(Base):
    __tablename__ = "user_pre_specification_profiles"
    __table_args__ = (
        UniqueConstraint("user_id", name="uq_user_pre_specification_profile"),
    )
