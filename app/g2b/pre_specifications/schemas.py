...
class PreSpecificationTransfer(BaseModel):
    """Stable hand-off contract: bfSpecRgstNo is the source identity."""

    model_config = ConfigDict(populate_by_name=True, extra="ignore")

    bf_spec_rgst_no: str = Field(validation_alias=AliasChoices("bf_spec_rgst_no", "bfSpecRgstNo"), min_length=1, max_length=80)
    bid_notice_no: str | None = None
    bid_notice_ord: str | None = None
    reference_no: str | None = Field(default=None, validation_alias=AliasChoices("reference_no", "refNo"))
    business_name: str | None = Field(default=None, validation_alias=AliasChoices("business_name", "businessName"))
    ...
    @field_validator("bf_spec_rgst_no", "bid_notice_no", "bid_notice_ord", "reference_no", "business_name", "business_type", "demand_agency_name", "ordering_agency_name", "contact_name", "contact_phone", mode="before")
    @classmethod
    def clean_text(cls, value):
        text = str(value or "").strip()
        return text or None

    @field_validator("registered_at", "opinion_deadline", "delivery_deadline", mode="before")
    @classmethod
...
class PreSpecificationExportRequest(BaseModel):
